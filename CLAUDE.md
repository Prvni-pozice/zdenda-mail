...
Aktuální verze: **`v7.2-hitl-retriage-2026-06-21`** (kanonický zdroj: `src/zdenda_mail/rules.py`, funkce `classify(item)`).
...
- `v7.2-hitl-retriage-2026-06-21` — **re-triage složky HITL/unsure** (3417 → klasifikačně 3 unsure). Načteno +327 nových INBOX/Junk mailů (+63 Sent). Nové patterny z analýzy HITL (v7.2 supplement blok v `rules.py`): IMPORTANT (`mathesio.com`, `adw.cz`, `stampi.cz`, `inexia.cz`, `generaliceska.cz`, `uschovna.cz`, `80edays.com`), SW (`theinformation.com`, `shopcamp.cz`, `purple-trading.com`, `grada.cz`, `beehiiv.com`, `smba.cz`, `ostendorf-osma.cz`, `voitechacademy.com`, `milionchvilek.cz`), ENERGIE (`centropol.cz`), DEVELOP (`supabase.com`, `n8n.io`), ESHOPS (`bazos.cz`, `steampowered.com`, `print.cz`), DOPRAVA (`cpost.cz`, `gls-czech.com`), BANKS (`trading212.com`), nové `EXTRA_SPAM_DOMAINS` (serptiom/theinsurancequoter/networkada/digimank/servomotor/transport10/smilemakers/qronge), podmíněné bloky Vodafone a Apple (vyúčtování→invoice). Rezidual 2449 unsure rozhodnut LLM čtením těla (17 paralelních subagentů + 37 manual) a uložen pod verzí `llm-hitl-triage-2026-06-21` (vyhrává jako globálně nejnovější v `reapply`). Pozn.: `mssf.cz` (MS2021+ depeše = dotace) potvrzeno uživatelem jako extrémně důležité → IMPORTANT (už bylo). Nový report `open_loops.md` (otevřené smyčky „co dlužím").
